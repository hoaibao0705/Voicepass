...
% 
    ...
    % input validation
    if( isempty(vec) || isempty(Nw) || isempty(Ns) ), error( usage ); end; %bao loi neu 1 trong 3 thanh phan dau la rong
    if( min(size(vec))~=1 ), error( usage ); end; %bao loi neu vector khong phai 1 chieu
    ...
    if( ~isempty(padding) ) %neu padding khong bi bo trong
